...
def process_sampling_multiplier(sampling_dir, sampling_multiplier, memory_size, k, seed=None):
    ...
    return {
        'Sampling_Multiplier': sampling_multiplier,
        'Average_Accuracy': average_accuracy if accuracies else None,
        'Total_Time_Sec': total_time,
        'Peak_Memory_MB': peak_memory
    }

def main():
    parser = argparse.ArgumentParser(description="Train KNN on Multiple Sampling Multipliers")
    parser.add_argument('--sampled_base_dir', type=str, default='sampled_buckets', help='Base directory containing sampled bucket sets')
    ...
    sampling_dirs = [
        os.path.join(args.sampled_base_dir, d) for d in os.listdir(args.sampled_base_dir)
        if os.path.isdir(os.path.join(args.sampled_base_dir, d)) and d.startswith('sampling_')
    ]

    if not sampling_dirs:
        print(f"No sampling directories found in {args.sampled_base_dir}. Exiting.")
        return
    ...
    if summary_metrics:
        ...
        summary_output_dir = args.output_metrics_dir
...
if __name__ == "__main__":
    import pandas as pd  # Import here to avoid errors if the script is not fully executed
    main()
