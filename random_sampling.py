...
def sample_data(input_file, sampling_multiplier, output_dir, seed=None):
    if seed is not None:
        np.random.seed(seed)
    ...
    min_flows_per_bucket = 1  # Minimum number of flows per bucket after sampling
    ...
    total_sampled_rows = int(total_rows * sampling_multiplier)  # e.g., 0.5 for 50%
    ...
    print(f"Sampling Multiplier: {sampling_multiplier}")
    ...
    print(f"Sampling completed for multiplier: {sampling_multiplier}\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Random Sampling for Continual Learning")
    ...
    parser.add_argument('--output_base_dir', type=str, default='sampled_buckets', help='Base directory to save sampled data')
    parser.add_argument('--seed', type=int, default=None, help='Random seed for reproducibility')

    args = parser.parse_args()

    for multiplier in args.sampling_multipliers:
        output_dir = os.path.join(args.output_base_dir, f'sampling_{multiplier}')
        try:
            sample_data(args.input_file, multiplier, output_dir, seed=args.seed)
